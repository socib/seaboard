/*
 * L.TimeDimension.CircleLabelMarker: circleMarker + divIcon where
 */
...
L.TimeDimension.Layer.CircleLabelMarker = L.TimeDimension.Layer.extend({
    ...
    _loadDataForTime: function(time, callback) {
        if (!this._dataLayer || !this._map) {
            return;
        }
        var point = this._baseLayer._point;
        var url = this._dataLayer.url + '?SERVICE=WMS&VERSION=1.1.1&REQUEST=GetFeatureInfo&SRS=EPSG:4326';
        url = url + '&LAYER=' + this._dataLayer.params.layers;
        url = url + '&QUERY_LAYERS=' + this._dataLayer.params.layers;
